...
| `bio` | **Required** on `POST` / `PUT /me`: trimmed length **100–2000 Unicode code points** (`utf8.RuneCountInString` / `sharedutils.CountRunes` after `TrimSpace` in `validateSubmitProfileFields`). **Not** UTF-8 byte length. Become-instructor form section 2 collects `bio`; FE Zod + live counter use the same code-point unit (`unicodeCodePointLength`) |
| `teaching_content_ideas` | **Required** on `POST` / `PUT /me`: trimmed length **50–500 Unicode code points** (same rune/code-point unit as `bio`). Become-instructor form **section 3** collects concrete class/course ideas and outlines (not topic-group taxonomy). Persisted on `instructor_applications` and `instructor_profiles` (same as `bio`); approve copies via `ProfilePayload`. FE Zod + counter mirror the same bounds |
