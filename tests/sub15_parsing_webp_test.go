...
// Sub 15 regression tests for ImageSizeFromPayload.
//
// Root-cause fix: pkg/logic/utils/parsing.go now blank-imports
// _ "golang.org/x/image/webp" so that image.DecodeConfig can decode WebP
// payloads produced by the bimg/CGO encode pipeline (sub11).  Without that
// import, image.DecodeConfig returned (0, 0) for every WebP payload.
//
// Convention reminder (docs/patterns.md):
//   All tests live under repository root `tests/`; never colocated with
//   production packages.
...
// TestImageSizeFromPayload_WebP_Regression is the primary regression for sub15:
// the upload pipeline (sub11) encodes images to WebP via bimg before storing.
// Before the fix, image.DecodeConfig had no WebP decoder registered, so width
// and height were always 0 for WebP payloads.
//
// This test encodes a PNG → WebP using EncodeWebP (the same function used by
// the upload pipeline). When CGO / libvips is unavailable, EncodeWebP returns
