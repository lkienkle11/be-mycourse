...
| `bio` | **Required** on `POST` / `PUT /me`: value is **Quill Delta JSON** stored in `TEXT` (same pattern as course `about_course`; **no migration**). **Structural allow-list** via `validateBioDelta` inside `validateProfile` (**every** `ProfilePayload` write path: application submit/resubmit **and** profile upsert `POST`/`PUT /me`): when the raw value parses as JSON with an `ops` **array**, every op must have a **string** `insert` and only allow-listed attributes (`bold`, `italic`, `underline`, `strike`, `list`); media embeds (`image`/`video`/`document` object inserts), `font`, `size`, `header`, and any unknown attribute → `ErrInvalidApplicationPayload`. Raw value larger than **32 KB** is rejected. JSON **without** an `ops` array (and non-JSON) is treated as **legacy plain text** — same rule as FE `coerceToDelta`, so FE and BE never disagree on the payload class. Length **100–2000 Unicode code points** counted on **extracted plain text** via `sharedutils.CountDeltaRunes` in `validateSubmitProfileFields` (uses `CountRunes` on string inserts, **including** whitespace; JSON without `ops` array falls back to `CountRunes(TrimSpace(raw))`). **Not** UTF-8 byte length; **not** `CountDeltaNonWhitespace`. Become-instructor form section 2 uses FE `DeltaEditor` (`allowMediaEmbed={false}`, `lockSystemFont` — no font/size/**header** controls); FE Zod + live counter use `countDeltaCodePoints` / plain extract + `unicodeCodePointLength`; FE sanitizes stored values on form hydrate (`sanitizeLockedTextDelta`) |
| `teaching_content_ideas` | **Required** on `POST` / `PUT /me`: trimmed length **50–500 Unicode code points** via `CountRunes` on the plain string (same **unit** — Unicode code points — as `bio`, but **not** Delta: still plain `Textarea`). Become-instructor form **section 3** collects concrete class/course ideas and outlines (not topic-group taxonomy). Persisted on `instructor_applications` and `instructor_profiles` (same as `bio`); approve copies via `ProfilePayload`. FE Zod + counter mirror the same bounds |
...
`applicationRow` and `profileRow` in `internal/instructor/infra/rows.go` embed exported `ProfileDataRow` with **`gorm:"embedded"`** so GORM `Create`/`Save` writes inline profile columns (`current_job_title_id`, `bio`, `teaching_content_ideas`, `cv_file_id`, …) on `instructor_applications` / `instructor_profiles`. Without exported embed + tag, GORM inserts only review metadata and PostgreSQL rejects the row (`current_job_title_id` NOT NULL) → HTTP 500 / `code:9001`.
