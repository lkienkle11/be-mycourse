...
// PermissionChecker is implemented by the RBAC service/use-case and injected into the
...
// Falls back to a DB query via the injected checker when the JWT context does not carry
// the full permission set.
func RequirePermission(checker PermissionChecker, actions ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		okAll, missing, err := checker.UserHasAllPermissions(userID, actions)
		if err != nil {
			response.AbortFail(c, http.StatusInternalServerError, errors.InternalError, "permission check failed", nil)
			return
		}
		if !okAll {
			response.AbortFail(c, http.StatusForbidden, errors.Forbidden, "forbidden: missing permission "+missing, nil)
			return
		}
		c.Next()
	}
...
func jwtPermissionsSatisfied(c *gin.Context, actions []string) bool {
	...
	for _, action := range actions {
		if _, has := permSet[action]; !has {
			response.AbortFail(c, http.StatusForbidden, errors.Forbidden, "forbidden: missing permission "+action, nil)
			return true
		}
	}
	c.Next()
