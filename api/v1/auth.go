...
var (
	accessTokenMaxAge  = int(services.AccessTokenTTL.Seconds())
	refreshTokenMaxAge = int(services.RefreshTokenTTL.Seconds())
)
...
func login(c *gin.Context) {
	...
	at, rt, err := services.Login(req.Email, req.Password)
	if err != nil {
		switch {
...
func confirmEmail(c *gin.Context) {
	...
	at, rt, err := services.ConfirmEmail(tok)
	if err != nil {
		switch {
...
// setAuthCookies writes the access and refresh JWTs as HttpOnly cookies.
