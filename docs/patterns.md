...
- GORM / JSONB **column** types for `models/*.go` fields: refresh-session JSONB **`Valuer`/`Scanner`** in **`pkg/gormjsonb/auth`**, soft-delete **`DeletedAt`** alias in **`pkg/sqlmodel`**.
