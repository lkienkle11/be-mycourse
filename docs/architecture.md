...
| Package | Purpose |
|---------|---------|
| `internal/shared/db/` | GORM setup, PostgreSQL connection (`search_path` per connection via `SCHEMA_NAME_APP` / default `public`, `tunePool` on `database/sql` after open), SQL migrations |
