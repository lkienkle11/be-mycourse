...
func requireJWT(c *gin.Context) bool {
	...
	if tok == "" {
		response.AbortFail(c, http.StatusUnauthorized, apperrors.Unauthorized, "missing bearer token", nil)
		return false
	}

	secret := setting.AppSetting.JWTSecret
	if secret == "" {
		response.AbortFail(c, http.StatusInternalServerError, apperrors.InternalError, "jwt not configured", nil)
		return false
	}
	...
	if err != nil {
		if stderrors.Is(err, jwt.ErrTokenExpired) {
			...
			response.AbortFail(c, http.StatusUnauthorized, apperrors.Unauthorized, "token expired", nil)
		} else {
			response.AbortFail(c, http.StatusUnauthorized, apperrors.Unauthorized, "invalid token", nil)
		}
		return false
	}
	if claims.UserID == 0 {
