...
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | `BIGINT GENERATED BY DEFAULT AS IDENTITY` | PK | RBAC catalog id (kept numeric) |
...
Product media (R2 files, Bunny Stream videos, local signed URLs, etc.).
...
| `r2_bucket_name` | `VARCHAR(255)` | NOT NULL DEFAULT `''` | R2 bucket name |
...
| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | `BIGINT GENERATED BY DEFAULT AS IDENTITY` | PK | Queue row id (kept numeric) |
| `provider` | `VARCHAR(16)` | NOT NULL | Target cloud (`R2`, `Bunny`, …) |
| `object_key` | `VARCHAR(512)` | NOT NULL DEFAULT `''` | R2 key when applicable |
...
|---------------|------------------|
| `latest_submission.profile.*` (headline, bio as Quill Delta JSON in TEXT / legacy plain, teaching_content_ideas, job/company, URLs, media IDs, portfolio, certificates) | Inline columns on `instructor_applications` (shared exported `ProfileDataRow` embedded in `applicationRow` with `gorm:"embedded"` in `internal/instructor/infra/rows.go`) |
...
| Profile columns | inline | `headline` (optional, default `''`; not on become-instructor form), `bio` (**required** on submit: Quill Delta JSON in `TEXT`, length 100–2000 Unicode code points on extracted plain text via `CountDeltaRunes`; structural allow-list via `validateBioDelta` on submit **and** profile upsert — string inserts only, attributes limited to `bold`/`italic`/`underline`/`strike`/`list`, media embeds + `font`/`size`/`header` rejected, raw ≤32 KB; legacy plain text OK; **no migration**), `teaching_content_ideas` (**required** on submit: plain text 50–500 Unicode code points via `CountRunes` — **`000033`**; concrete class/course ideas, not topic taxonomy), `years_of_experience` (enum), `current_job_title`, **`current_job_title_id`** (NOT NULL), `current_company`, **`current_company_id`** (nullable), **`current_company_domain`** (nullable), **`current_company_description`** (nullable), **`current_company_location`** (nullable), `cv_file_id`, URLs, `portfolio_links` JSONB, `certificates` JSONB (`title`, `issuer`, `issued_year`, `credential_url?`, `certificate_file_id?`), `intro_video_file_id` |
