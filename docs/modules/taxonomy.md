...
`services/taxonomy/fields.go` imports **`mycourse-io-be/pkg/taxonomy`** (alias `taxonomypkg`) so HTTP `status` strings are normalized before repository writes. **Taxonomy** HTTP handlers (`api/v1/taxonomy/*_handler.go`) delegate list/create/update payloads to **`mapping.CategoryListHTTPPayload`**, **`CategoryRowHTTPPayload`**, **`TagListHTTPPayload`**, **`TagRowHTTPPayload`**, **`CourseLevelListHTTPPayload`**, **`CourseLevelRowHTTPPayload`** so **`api/`** never imports **`models`** (depguard `restrict_api`). **`services/taxonomy/*.go`** return **`models.Category`** / **`models.Tag`** / **`models.CourseLevel`**.
