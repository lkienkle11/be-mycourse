...
func TestParseImageURLForOrphanCleanup_Empty(t *testing.T) {
	_, _, _, ok := mediainfra.ParseImageURLForOrphanCleanup("")
	if ok {
...
func TestParseImageURLForOrphanCleanup_Local(t *testing.T) {
	prov, key, bID, ok := mediainfra.ParseImageURLForOrphanCleanup("/api/v1/media/files/local/abc123token")
	if !ok {
...
func TestParseImageURLForOrphanCleanup_External(t *testing.T) {
	_, _, _, ok := mediainfra.ParseImageURLForOrphanCleanup("https://external.cdn.example.com/images/photo.jpg")
	if ok {
...
func TestScanJSONBForImageURLs_Empty(t *testing.T) {
	urls := mediainfra.ScanJSONBForImageURLs(nil)
	if len(urls) != 0 {
...
func TestScanJSONBForImageURLs_FlatObject(t *testing.T) {
	...
	urls := mediainfra.ScanJSONBForImageURLs(raw)
