...
- Bulk collaborator management **MUST** run in a single transaction, with batch instructor/existing-collaborator lookups and one batched `RoleBindingService.Assign` call for every newly-added principal (`internal/authorization`'s resource-scoped role gate, not a Course-owned table), not N per-user insert/update round-trips.
...
- The system **MUST** expose only FE-facing media response fields on `dto.UploadFileResponse`: `id`, `user_id`, `display_name`, `visibility`, `kind`, `filename`, `mime_type`, `size_bytes`, `status`, `url`, `object_key`, `video_id`, `thumbnail_url`, `embeded_html`, `duration`, typed `metadata`, `row_version`, `created_at`, `updated_at`. Bunny/provider-internal delivery fields (`bunny_video_id`, `bunny_library_id`, `direct_play_url`, `hls_playlist_url`, `preview_animation_url`, `video_provider`, `content_fingerprint`, `r2_bucket_name`) **MUST NOT** be serialized to public JSON and remain server-side (persisted in `media_files` / `metadata_json` as needed). **`MEDIA_BUNNY_STREAM_CDN_HOSTNAME`** **MUST** be configured for CDN-backed thumbnail URLs.
