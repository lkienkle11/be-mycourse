...
Pushing to **`master`** triggers `.github/workflows/deploy-dev.yml`: the test job installs **libvips** + **HDF5** dev packages and **pkg-config** on the runner (fast APT mirror + package cache), runs **`go test`** (including **`CGO_ENABLED=1`**), **`go vet`**, and **`golangci-lint run`**; the **build** job produces **`mycourse-io-be-dev`**, then **`deploy`** `rsync`s to the server and runs `scripts/pm2-reload-with-binary-rollback.sh`. For a local stripped binary matching CI, use **`go build`** with **`CGO_ENABLED=1`** or **`make build`**. See [`docs/deploy.md`](docs/deploy.md) for the full runbook.
