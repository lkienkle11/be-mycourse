...
- Course authoring: `internal/course` owns draft lifecycle, collaborator membership, review transitions, outline stable IDs, and learner progress. Repository access checks go through one seam, `requireCourseAction` (`internal/course/infra/repo_access.go`), which calls `internal/authorization`'s `Authorizer` with `CoursePolicyProvider` registered; the canonical owner is synthesized (no stored binding) and EDITOR collaborator membership is an `authorization_role_bindings` row, not a `course_collaborators` table (dropped by migration `000037`). **Prepare draft**, **submit for review**, and **reopen draft** remain canonical-owner-only (`requireOwnerAccess` → `ErrCourseOwnerOnly`).
